...
\lstdefinestyle{customc}{
	...
	commentstyle=\fontfamily{qcs}\itshape\color{purple!40!black},
...
\begin{wrapfigure}[8]{r}{.34\textwidth}
	\vspace{-15pt}
...
The Whiley language uses a structural type system : types are determined by their declaration instead of their identifiers. Two nominal types with different names can thus have the same values.\\

In such a system, types can usefully be seen as a subset of the set of correct values $\val$ in language. Type operations can then be interpreted as they equivalent on sets. The sub-typing operator, that tells if a type is a sub-type of an other one, is define as the set inclusion.\\
...
The type on each branch are constructed using negation and intersection operators. 
...
Wyc handles lexing, parsing and type checking of Whiley sources. It also fully resolves names, giving each variable of function a unique identifier. 
...
\begin{figure}[h]
	...
	\node[stateB,align=center] (E) at (13.6,0) {Java\\ Bitecode};
...
This structure greatly simplify the work of back-end compilers that have only to deal with well formed and typed programs. 
...
\begin{itemize}
	\item Each file is compile to a unique VHDL file
	\item Each function is compile to a unique VHDL entity
\end{itemize}

\paragraph{Restrictions on the language}\pindent

Only a small subset of the Whiley language is supported by the compiler. The limitations arise from two major reasons :
...
\textbf{\textsl{Remark :}} The case of Whiley's \code int type is resolved by fixing the size of integer (32 bits signed integers). To preserve Whiley semantic, an analysis of constraints of integer types could replace this solution, that would enable to adjust precisely the size of integers.
...
Because some type constructions can't be easily represented with VHDL types (intersections, negations), types must be simplify when feasible to equivalent forms free of unsupported operations. \\
...
\begin{figure}[h]
	...
	\hline
	$\cup(\cap($int$))$ & $\cup(\cap($int$),~\cap($int$))$ & 
	\begin{tabular}{lll}
...
\begin{figure}[h]
	...
	draw,processblue , text=black , minimum width =1 cm] (A) at (-.5,0) {$T$};
	
	\node[primitive] (B) at (2,-.2) {pri};
	\node[leaf] (BA) at (4.5,-.4) {has : bool};
	...
	\normalsize
	\end{tikzpicture}
	\caption{Representation of type $T$ : \{int a, byte b\}$|$\{int b\}$|$null }
\end{figure}


\textbf{\textsl{Remark :}} Recursive types such as trees are not convertible as they describe values of unknown and unbound size.
...
\begin{enumerate}
	\item \textbf{Building of the right hand side} : A tree representing the \fcolorbox{processred}{processred!10}{new value} is built by compiling the expression.
	\item \textbf{Adjusting types} : Some \fcolorbox{processblue}{processblue!10}{additional signals} can be created to make the type of the expression fit the left hand side one.
	\item \textbf{Modifying trees} : Building the global tree of the modified variable, keeping some \fcolorbox{processpurple}{processpurple!10}{old sub-trees} and building \fcolorbox{processgreen}{processgreen!10}{new inner nodes}.
\end{enumerate}
...
\begin{wrapfigure}[18]{r}{0.28\textwidth}
	\center
\vspace{-22pt}
...
\autoref{Dloops} details the additional logic build around loop's body and condition in a simple case where they are supposed to be ready at the same time.  This diagram shows that a extra iteration is computed, that is dropped if not necessary. The results of the previous iteration (or the inputs) are stored in a buffer waiting for the condition value to be ready.
...
\begin{wrapfigure}[22]{r}{.38\textwidth}
\vspace{-15pt}
\center
	\begin{tikzpicture}[
